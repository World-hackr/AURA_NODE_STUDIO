...
$HostRemoteRoot = Split-Path -Parent $TestsRoot
$RepoRoot = Split-Path -Parent $HostRemoteRoot
...
function Get-PlatformIoExecutable {
    ...
    throw "PlatformIO was not found. Install the PlatformIO IDE extension in VS Code, then try again."
}

function Resolve-ProjectRoot([string]$RequestedProjectPath) {
    if ([System.IO.Path]::IsPathRooted($RequestedProjectPath)) {
        return $RequestedProjectPath
    }

    return [System.IO.Path]::GetFullPath((Join-Path $TestsRoot $RequestedProjectPath))
}
...
function Invoke-PlatformIo([string]$ProjectRoot, [string[]]$Arguments) {
...
$resolvedProjectRoot = Resolve-ProjectRoot $ProjectPath

if (-not (Test-Path (Join-Path $resolvedProjectRoot "platformio.ini"))) {
    throw "No platformio.ini was found in $resolvedProjectRoot"
}
...
switch ($Action) {
    ...
    "build" {
        Invoke-PlatformIo $resolvedProjectRoot @("run", "-e", $Environment)
        break
    }
    "upload" {
        ...
        Write-Host ""
        Invoke-PlatformIo $resolvedProjectRoot @("run", "-e", $Environment, "-t", "upload", "--upload-port", $resolvedPort)
        break
    }
    "monitor" {
        $resolvedPort = Resolve-UploadPort $Port
        ...
        Write-Host ""
        Invoke-PlatformIo $resolvedProjectRoot @("device", "monitor", "-b", "115200", "--port", $resolvedPort)
        break
    }
    "upload-monitor" {
        $resolvedPort = Resolve-UploadPort $Port
